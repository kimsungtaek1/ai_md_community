...
export class PgRepository implements IRepository {
  ...
  static async create(databaseUrl: string): Promise<PgRepository> {
  ...
  private async migrate(): Promise<void> {
    const schemaPath = join(process.cwd(), "db", "postgres_schema.sql");
  ...
  private async writeAudit(
    ...
    await client.query(
      ...
      [nanoid(), eventType, entityType, entityId, actorAgentId ?? null, JSON.stringify(payload), nowIso()]
    );
  }
  ...
  private async findExactDuplicatePostRows(client: PoolClient, input: {
    ...
    return rows as Array<{ id: string; created_at: string }>;
  }
  ...
  private async readPostsFrom(client: PoolClient): Promise<Post[]> {
    ...
    for (const row of debateRes.rows) {
      const mapped: DebateTurn = {
        ...
        stance: row.stance, message: row.message, createdAt: String(row.created_at)
      };
  ...
  async createAgent(name: string): Promise<Agent> {
    return this.withTransaction(async (client) => {
      const agent: Agent = { id: nanoid(), name, createdAt: nowIso() };
      await client.query(
  ...
  async createCategoryRequest(input: {
    ...
    return this.withTransaction(async (client) => {
      ...
      const request: CategoryRequest = {
        id: nanoid(), requestedBy: input.requestedBy, name: input.name,
    ...
    return this.withTransaction(async (client) => {
      ...
      await client.query(
        `INSERT INTO category_request_reviews (id, request_id, agent_id, decision, reason, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [nanoid(), requestId, input.agentId, input.decision, input.reason, nowIso()]
      );
      ...
      const approvals = countRows[0].approvals as number;
      const rejects = countRows[0].rejects as number;
    ...
    return this.withTransaction(async (client) => {
      ...
      if (duplicates.length > 0) {
        ...
        for (const duplicateId of duplicateIds) {
          await client.query(`DELETE FROM posts WHERE id = $1`, [duplicateId]);
        }
      ...
      const now = nowIso();
      const post: Post = {
        id: nanoid(), categoryId: input.categoryId, title: input.title, body: input.body,
  ...
  async recordPostView(postId: string, input: {
    ...
    return this.withTransaction(async (client) => {
      ...
      const stats = rows[0] as { view_count: number; unique_viewer_count: number };
      return {
        postId,
  ...
  async addComment(postId: string, input: { agentId: string; body: string }): Promise<Comment> {
    return this.withTransaction(async (client) => {
      await this.requireAgent(client, input.agentId);
      await this.requirePostRow(client, postId);
      const comment: Comment = { id: nanoid(), agentId: input.agentId, body: input.body, createdAt: nowIso() };
      await client.query(
      ...
      await client.query(`UPDATE posts SET updated_at = $1 WHERE id = $2`, [nowIso(), postId]);
  ...
  async createRevisionRequest(
    ...
    return this.withTransaction(async (client) => {
      ...
      const revision: RevisionRequest = {
        id: nanoid(), reviewerAgentId: input.reviewerAgentId,
  ...
  async addDebateTurn(
    ...
    return this.withTransaction(async (client) => {
      ...
      const turn: DebateTurn = {
        id: nanoid(), speakerAgentId: input.speakerAgentId,
  ...
  async applyRevisionDecision(
    ...
    return this.withTransaction(async (client) => {
      ...
      if (decision.accepted) {
        await client.query(`UPDATE posts SET body = $1, updated_at = $2 WHERE id = $3`, [
